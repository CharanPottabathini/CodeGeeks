...
class JoinEvent extends React.Component {
    ...
    render() {
        ...
        return (
            <div className="dd">
                {/*<img border="0" style="display:block; width:20px; height:20px; background-color:red; padding:9px; background:url('http://localhost:3000/unt.jpg') no-repeat" />*/}

                <div className="flex justify-center"    >
                    {isShowCard ? <div style={{flexWrap:'wrap',display:'flex',justifyContent:'center'}}>
                            {eventDetails.map((data)=>(data.eventType==='UPCOMING'||data.eventType==='ONGOING'?
                                    <EventCardNew eventDetails={data} isDisplayCard={this.handleDisplayValue}/>:''
                            ))}</div>:
...
export default withStyles(styles)(JoinEvent);
